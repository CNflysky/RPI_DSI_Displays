...
open terminal,install `raspberrypi-kernel-headers`:
```bash
sudo apt install raspberrypi-kernel-headers
```
then create a folder named `w280bf036i`:
```bash
mkdir w280bf036i
```
write our source code at here(`panel-wlk-w280bf036i.c`):
```c
/*
...
```
Now we have finished our source code,but there are some notable points here:  
in `static void w280bf036i_init_sequence`，we filled panel's initial code sequences:
```c
static void w280bf036i_init_sequence(struct w280bf036i *w280bf036i) {
...
```
You can contact panel manufacturer to obtain them.  
then in `w280bf036i_mode` struct，we filled panel's porch:
```c
static const struct drm_display_mode w280bf036i_mode = {
    .clock = 25000,
...
make
```
when compliation finished,there will be multiple file appeared in the current directory,make sure there is a `panel-wlk-w280bf036i.ko` in them.  
# Device tree
`vc4-kms-dsi-w280bf036i.dts`:
```
...
```
~~My converter is designed as I2C SCL used as RST pin,so I select GPIO 45 as reset-gpios.~~   
*P.S. this design can make troubles. Should add a I2C-GPIO chip as panel io controller(e.g.`PCA9536`)。*  
*P.P.S. Although my board are arrived，but chips aren't arrived yet，so i enabled I2C to make RST a HIGH-Level to prevent panel from resetting itself.to be continued...*  
# Apply
```bash
